 ...
 * @LastEditTime: 2022-03-17 23:02:44
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: \sd365-tms-omp-front-end\src\views\basedata\position\index.vue
-->

<template>
  <el-container>
    <el-header style="margin-left: -80px">
      ...
      >
        ...
        <el-form-item label="职位">
          <el-input v-model="positionQuery.name" style="width: 200px" />
        </el-form-item>
        <el-form-item label="编号">
          <el-input v-model="positionQuery.code" style="width: 200px" />
        </el-form-item>
      </sd-QueryPanel>
    </el-header>
    <el-main>
      ...
      >
        ...
        <!-- <el-table-column sortable align="center" prop="code" label="职位编号" width="105" /> -->
        <el-table-column  align="center" prop="name" label="职位名称" sortable :sort-method="(a,b) => a.name.localeCompare(b.name)" />
        ...
        <el-row>
          <el-col :span="11">
            <el-form-item label="职位名称" prop="name">
              <el-input v-model="positionVO.name" @input="handlerChange"/>
            </el-form-item>
          </el-col>
          <el-col :span="11" style="margin-left: 20px">
            <el-form-item label="职位编号" prop="code">
              <el-input v-model="positionVO.code" placeholder="" @input="handlerChange"/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="11">
            <el-form-item label="备注" prop="remark">
              <el-input v-model="positionVO.remark" placeholder="" />
            </el-form-item>
          </el-col>
        </el-row>
      </sd-FormDialog>
...
export default {
  ...
  data() {
    ...
    return {
       ...
       * 查询区模型
       */
      positionQuery: { name: '', account: '' }, // 查询对象
      /**
       * 展示区模型
       */
      positions: [], // 从vo取得的客户信息
  ...
  methods: {
    ...
    handleSizeChange(val) {
      ...
      this.queryposition(this.commonPage.pageSize, this.commonPage.currentPage)
    },
     ...
     * @return {*}
     */
    queryposition(pageSize, currentPage) {
      this.dialogVisible = false
      this.queryLoading = true
      this.positionQuery.pageSize = pageSize
      this.positionQuery.pageNum = currentPage
      commonQuery(this.positionQuery)
        .then((response) => {
          // eslint-disable-next-line no-unused-vars
         ...
         */
          this.positions = response.data // 分页信息带回显示
    ...
    handleRowDoubleClick(row, column, event) {
      querypositionById(row.id).then((response) => {
        // 取得CommonResponse的Body
        if (response != null) {
          // eslint-disable-next-line no-undef
          this.positionVO = response
          // eslint-disable-next-line no-undef
   ...
   /**
     ...
     */
    onBtnBatchDelete() {
      const positionVOS = this.$refs.multipleTable.selection
      if (positionVOS == null || positionVOS.length === 0) {
        this.$message.error('至少选择一条进行操作')
        return
      }
      const deletepositionDTOS = new Array(positionVOS.length)
      for (let i = 0; i < positionVOS.length; i++) {
        deletepositionDTOS[i] = {
          id: positionVOS[i].id,
          version: positionVOS[i].version
        }
      }
      batchRemove(deletepositionDTOS)
    ...
    onDisableRecodClick(status) {
      ...
      const positionDTOS = new Array(positionVOS.length)
      for (let i = 0; i < positionVOS.length; i++) {
        positionDTOS[i] = { id: positionVOS[i].id, status: status }
      }
      // eslint-disable-next-line no-array-constructor
      batchUpdate(positionDTOS)
        .then((response) => {
          if (response) {
            this.$message.success('操作成功')
            this.onBtnQueryClick()
    ...
    onBtnDeleteClick(index, row) {
      const currentpositionVO = this.positions[index]
      ...
      this.$confirm('确定删除记录吗').then((result) => {
        // 获取多条ID 调用服务端
        remove(currentpositionVO.id, currentpositionVO.version)
          .then((response) => {
    ...
    onBtnEditClick(index, row) {
      ...
      querypositionById(currentpositionVO.id)
        .then((response) => {
          // 取得CommonResponse的Body
      ...
      // if (this.positionVO.settleTypeDTO != null) {
      //   this.loadSettleType(this.positionVO.settleTypeDTO.id)
      // } else {
     ...
     * @return {*}
     */
    showposition(positionVO) {
      this.copyLaoding = true
      // this.loadOrganizations()
      // this.loadCompanies()
      // 调用 api 插入记录并且返回记录
      const selectedposition = positionVO
      console.log(selectedposition)
      querypositionById(selectedposition.id)
        .then((response) => {
          // eslint-disable-next-line no-undef
          // 加载下拉框数据 设置下拉框的选项
    ...
    onBtnSaveClick(source) {
      ...
      if (this.dialogType === 'ADD') {
        const currentpositionDTO = this.positionVO
        currentpositionDTO.creator = this.$store.getters.name
        currentpositionDTO.modifier = this.$store.getters.name
        console.log(currentpositionDTO)
        add(currentpositionDTO)
          .then((response) => {
            if (response) {
              // 显示保存成功并且返回
              this.$message.success('操作成功')
          ...
          .catch(() => {
            ...
            this.positionVO = null
            this.$message.error('网络异常')
          })
      } else if (this.dialogType === 'EDIT') {
        this.positionDTO = this.positionVO
        this.positionDTO.modifier = this.$store.getters.name
        modify(this.positionDTO)
          .then((response) => {
            // 显示保存成功并且返回
            this.$message.success('操作成功')
            this.dialogVisible = false
        ...
        if (this.isRewriteEditor) {
          const currentpositionDTO = this.positionVO
           console.log("currentpositionDTO",currentpositionDTO)
             add(currentpositionDTO).then((response) => {
            ...
            .catch(() => {
            this.$message.error('职位名/职位编号重复！！')
          })
        } else {
          this.$message.warning("请修改职位名称和职位编号后再提交")
        }
